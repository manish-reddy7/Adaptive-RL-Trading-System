...
TICKER = "^NSEI"
OUTPUT_FILE = "nifty50_price_regime_overlay_60d.png"
SEED = 42
DISPLAY_DAYS = 60
...
def detect_regimes(df: pd.DataFrame, n_regimes: int = 3, lookback: int = 21) -> pd.DataFrame:
    ...
    features = StandardScaler().fit_transform(work[["log_return", "volatility"]].values)
...
def plot_nifty_regime_overlay(price_df: pd.DataFrame, regime_df: pd.DataFrame) -> None:
    display_price_df = price_df.tail(DISPLAY_DAYS).copy()
    display_regime_df = regime_df[regime_df["date"] >= display_price_df["date"].iloc[0]].copy()

    fig, ax = plt.subplots(figsize=(14, 7.5))

    segments = build_segments(display_regime_df)
    for start, end, regime in segments:
        style = REGIME_STYLES[regime]
        ax.axvspan(start, end, color=style["color"], alpha=style["alpha"], lw=0)

    ax.plot(
        display_price_df["date"],
        display_price_df["close"],
        color="#111111",
        linewidth=1.9,
